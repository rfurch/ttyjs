...
Server.prototype.initRoutes = function() {
  ...
  this.get('/option.js', function(req, res, next) {
    return self.handleOptions(req, res, next);
  });
};
...
Server.prototype.handleConnection = function(socket) {
  ...
  // RFurch we receive now an extra argument, containerName
  socket.on('create', function(cols, rows, containerName, func) {

	// RFurch.  Transfer containerName 
	console.log(containerName);	
    return session.handleCreate(cols, rows, containerName, func);
  });
  ...
  socket.on('parameter', function(id, data) {
    console.log(data);
//    return session.handleData(id, data);
  });
...
// RFurch:  receive containerName, added (basic) functionality!
Session.prototype.handleCreate = function(cols, rows, containerName, func) {
  var self = this
    , terms = this.terms
  ...
  // RFurch:  we 'hardcoded' shell and it's arguments 
  // (to compose something like:  lxc-attach -n lab0001_PC1  )
  shell='lxc-attach';
  shellArgs=["-n", containerName];
