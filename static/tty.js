...
tty.open = function() {
  ...
  tty.elements = {
	...
	whichOne: document.getElementById('whichOne')    // HTML Tag
  };
  ...
  whichOne = tty.elements.whichOne;   // now it's a TTY property!

  if (open) {
    
    on(open, 'click', function() {
	//alert(whichOne.value);
    new Window(whichOne.value);   // transfer to WINDOW
    });
  }
...
// RFurch
// Window now receives 'containerName' which represents the container's name
// It will be, a window's property  
// e.g:   in lxc-attach -n PC1,  containerName will be:  'PC1'

function Window(containerName, socket) {                 
  var self = this;
  ...
  // RFurch
  // now, every window will have a property, which is the name of the container to 
  // attach, whose pty will be presented to end user
  this.containerName = containerName || 'none';
...
function Tab(win, socket) {
  ...
  // RFurch. CREATE event is modified to send to server a new parameter: containerName !!
  
//  this.socket.emit('create', cols, rows, 'lab001_PC_2', function(err, data) {
  this.socket.emit('create', cols, rows, win.containerName, function(err, data) {
...
//    this.window.title.innerHTML = name;
    ...
    this.window.title.innerHTML = 'VirLabNet';
  }
};
